  ...
  const createExpense = async () => {
    ...
    try {
      ...
      if (
        ...
        ["TOLL", "PARKING", "REPAIR", "OTHER"].includes(addFormData.category)
      ) {
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600">
                  {t("stats.fuelExpenses")}
                </p>
                <p className="text-2xl font-bold text-blue-600">
                  {stats?.byType?.FUEL?.count}
                </p>
                <p className="text-xs text-gray-500 mt-1">
                  {formatCurrency(stats?.byType?.FUEL?.amount || 0)}
                </p>
              </div>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600">
                  {t("stats.miscExpenses")}
                </p>
                <p className="text-2xl font-bold text-green-600">
                  {stats?.byType?.MISC?.count}
                </p>
                <p className="text-xs text-gray-500 mt-1">
                  {formatCurrency(stats?.byType?.MISC?.amount || 0)}
                </p>
              </div>
              <div className="w-12 h-12 bg-green-100 rounded-lg flex items-center justify-center">
                <svg
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            <div>
              <span>{t("table.allExpenses")}</span>
              {filteredExpenses.length > 0 && (
                <span className="text-sm font-normal text-gray-500 ml-2">
                  ({startIndex + 1}-
                  {Math.min(endIndex, filteredExpensesPaginationData?.total)} of{" "}
                  {filteredExpensesPaginationData?.total})
                </span>
              )}
            </div>
          ...
          {filteredExpenses.length > 0 ? (
            <>
              ...
              <div className="block lg:hidden">
                ...
                <div className="horizontal-scroll-wrapper mobile-scroll">
                        ...
                        <div className="flex items-center space-x-4">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-400 mt-1">
                              ...
                              • Driver:{" "}
                              {typeof expense.driverId === "object" &&
                  ...
                  <div className="flex items-center space-x-4">
                    <span className="text-sm text-gray-600">
                      {t("table.showingResults", {
                        start: startIndex + 1,
                        end: Math.min(endIndex, filteredExpenses.length),
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">
                {expenses.length === 0
                  ? t("table.noExpenses")
                  : t("table.noMatchingExpenses")}
              </h3>
              <p className="text-gray-500 mb-6">
                {expenses.length === 0
                  ? t("table.noExpensesMessage")
                  : t("table.noMatchingMessage")}
              </p>
              {expenses.length === 0 ? (
                <Button>
                  <svg
                  ...
                  {isEditMode ? (
                    <input
                      ...
                      onChange={(e) =>
                        setEditFormData({
                          ...editFormData,
                          amountFinal: parseFloat(e.target.value) || 0,
                        })
                      }
                      className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                    />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    ...
                    {(selectedExpense as any).confidence && (
                      <div>
                        ...
                        <div className="text-gray-600">
                          {Math.round(
                            (selectedExpense as any).confidence * 100
                          )}
                          %
                        </div>
                      </div>
              ...
              {selectedDriver && (
                <div className="bg-gray-50 p-4 rounded-lg">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-600 mb-1">
                        {t("modal.assignedVehicle")}
                      </label>
                      {selectedDriverVehicle ? (
                        <div>
                          <div className="text-gray-900 font-medium">
                            {selectedDriverVehicle.year}{" "}
                            {selectedDriverVehicle.make}{" "}
                            {selectedDriverVehicle.model}
                          </div>
                          <div className="text-sm text-gray-500">
                            {selectedDriverVehicle.licensePlate} • Current KM:{" "}
                            {selectedDriverVehicle.currentOdometer !==
                              undefined &&
                            selectedDriverVehicle.currentOdometer !== null
                              ? `${selectedDriverVehicle.currentOdometer.toLocaleString()} km`
                              : "N/A"}
                          </div>
                        </div>
                      ) : (
                  ...
                  <div>
                    ...
                    {ocrResult && ocrResult.confidence < 0.5 && (
                      <p className="text-sm text-amber-600 mt-1">
                        ⚠️ Low confidence OCR results - please verify the
                        extracted data
                      </p>
                    )}
                  </div>

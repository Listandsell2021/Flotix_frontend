...
const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <>
      ...
      {isImpersonating && impersonatedCompany && originalUser && (
        <div className="layout-stable fixed top-0 right-0 left-0 lg:left-64 z-50 bg-gradient-to-r from-amber-500 via-orange-500 to-red-500 text-white shadow-2xl">
          <div className="px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between h-14">
              ...
              <Button
                ...
                className="bg-white/20 hover:bg-white/30 backdrop-blur-sm text-white border-2 border-white/40 hover:border-white/60 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105"
                size="sm"
              >
                <svg
      ...
      <header
        className={cn(
          "layout-stable fixed right-0 left-0 lg:left-64 z-30 bg-white/85 backdrop-blur-xl border-b border-secondary-200/50 shadow-soft",
                ...
                >
                  ...
                  >
                    <path
                      ...
                      strokeLinejoin="round"
                      strokeWidth={2}
                      d="M15 17h5l-3.405-3.405A6.5 6.5 0 0118 10c0-3.314-2.686-6-6-6s-6 2.686-6 6a6.5 6.5 0 001.405 4.595L6 17h5m4 0v1a3 3 0 11-6 0v-1m6 0H9"
                    />
                  </svg>
                </button>
                  ...
                  <div className="absolute right-0 mt-3 w-80 max-w-[calc(100vw-2rem)] bg-white/95 backdrop-blur-xl rounded-xl shadow-large border border-secondary-200/50 py-2 z-50">
                    <div className="px-4 py-2 border-b border-secondary-100">
                      <span className="font-semibold text-secondary-900">
                        {t("dashboard.auditLogNotifications")}
                      </span>
                    </div>
                    <div className="max-h-96 overflow-y-auto">
                      {auditLoading ? (
                        ...
                        <div className="px-4 py-6 text-secondary-400 text-center">
                          {t("dashboard.noRecentAuditLogs")}
                        </div>
                      ) : (
                        <ul className="divide-y divide-secondary-100">
                          {auditLogs.map((log) => {
                            const date = new Date(log.timestamp);
                            const translatedAction = t(
                              `audit:actions.${log.action}`,
                              { defaultValue: log.action }
                            );
                            const translatedModule = t(
                              `audit:modules.${log.module}`,
                            ...
                            let translatedDetails =
                              log.details ||
                              `${translatedAction} ${translatedModule}`;
                            ...
                            if (log.details) {
                              ...
                              if (
                                log.details.includes("Failed login attempt") &&
                                log.details.includes("Account inactive")
                              ) {
                                translatedDetails = `${t(
                                  "audit:details.failed_login_inactive"
                                )}: ${email}`;
                              } else if (
                                log.details.includes("Failed login attempt") &&
                                log.details.includes("Invalid password")
                              ) {
                                translatedDetails = `${t(
                                  "audit:details.failed_login_password"
                                )}: ${email}`;
                              } else if (
                                log.details.startsWith("User login:")
                              ) {
                                translatedDetails = `${t(
                                  "audit:details.user_login"
                                )}: ${email}`;
                              }
                            }
                            ...
                            return (
                              <li
                                key={log._id}
                                className="py-3 px-4 flex flex-col gap-1 hover:bg-secondary-50 transition-colors"
                              >
                  ...
                  {showDropdown && (
                    <div className="absolute right-0 mt-3 w-64 max-w-[calc(100vw-2rem)] bg-white/95 backdrop-blur-xl rounded-xl shadow-large border border-secondary-200/50 py-2 z-50">
                      ...
                      <div className="py-2">
                        ...
                        >
                          <svg
                            className="w-4 h-4 mr-3 text-secondary-400"
                            fill="none"
                            stroke="currentColor"
                            viewBox="0 0 24 24"
                          >
                      ...
                      <div className="border-t border-secondary-100 pt-2">
                        ...
                        >
                          <svg
                            className="w-4 h-4 mr-3 text-accent-500"
                            fill="none"
                            stroke="currentColor"
                            viewBox="0 0 24 24"
                          >

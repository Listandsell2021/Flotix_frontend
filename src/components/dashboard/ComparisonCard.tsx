...
export default function ComparisonCard({ data }: ComparisonCardProps) {
  ...
  const formatPeriod = (period: { start: string; end: string }) => {
    const start = new Date(period.start).toLocaleDateString(i18n.language, {
      day: "numeric",
      month: "short",
    });
    const end = new Date(period.end).toLocaleDateString(i18n.language, {
      day: "numeric",
      month: "short",
      year: "numeric",
    });
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-blue-50 p-4 rounded-lg">
              ...
              <p className="text-xs text-blue-500 mt-1">
                {formatPeriod(data.period1.period)}
              </p>
            </div>
            <div className="bg-purple-50 p-4 rounded-lg">
              ...
              <p className="text-xs text-purple-500 mt-1">
                {formatPeriod(data.period2.period)}
              </p>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
              <div className="flex-1">
                ...
                <div className="flex items-center gap-4 mt-1">
                  <span className="text-sm text-blue-600">
                    {formatCurrency(data.period1.fuel)}
                  </span>
                  <span className="text-gray-400">→</span>
                  <span className="text-sm text-purple-600">
                    {formatCurrency(data.period2.fuel)}
                  </span>
                </div>
              </div>
              <div
                className={`flex items-center gap-1 ${getChangeColor(
                  data.changes.fuelChange
                )}`}
              >
            ...
            <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
              <div className="flex-1">
                ...
                <div className="flex items-center gap-4 mt-1">
                  <span className="text-sm text-blue-600">
                    {formatCurrency(data.period1.misc)}
                  </span>
                  <span className="text-gray-400">→</span>
                  <span className="text-sm text-purple-600">
                    {formatCurrency(data.period2.misc)}
                  </span>
                </div>
              </div>
              <div
                className={`flex items-center gap-1 ${getChangeColor(
                  data.changes.miscChange
                )}`}
              >
            ...
            <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
              <div className="flex-1">
                <p className="text-sm font-medium text-gray-700">
                  {t("comparison.avgExpense")}
                </p>
                <div className="flex items-center gap-4 mt-1">
                  <span className="text-sm text-blue-600">
                    {formatCurrency(data.period1.avgExpense)}
                  </span>
                  <span className="text-gray-400">→</span>
                  <span className="text-sm text-purple-600">
                    {formatCurrency(data.period2.avgExpense)}
                  </span>
                </div>
              </div>
              <div
                className={`flex items-center gap-1 ${getChangeColor(
                  data.changes.avgExpenseChange
                )}`}
              >

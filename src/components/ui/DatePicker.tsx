  ...
  const renderCalendar = () => {
    ...
    while (day <= endDate) {
      for (let i = 0; i < 7; i++) {
        ...
        const isRangeStart = mode === 'range' && rangeStart && isSameDay(day, rangeStart);
        const isRangeEnd = mode === 'range' && rangeEnd && isSameDay(day, rangeEnd);
        const isInRange =
          mode === 'range' &&
          rangeStart &&
          rangeEnd &&
        ...
        days.push(
          <button
            ...
            className={`
              ...
              ${isSelected || isRangeStart || isRangeEnd ? 'bg-primary-600 text-white hover:bg-primary-700' : ''}
  ...
  const getDisplayValue = () => {
    if (mode === 'single') {
      ...
      if (rangeStart && rangeEnd) {
        return `${format(rangeStart, 'P', { locale })} - ${format(rangeEnd, 'P', { locale })}`;
      } else if (rangeStart) {
